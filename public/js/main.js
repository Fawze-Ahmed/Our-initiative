...
async function checkApiStatus() {
  const isHealthy = await window.apiHealth();
  
  if (!isHealthy) {
    console.warn('⚠️ تحذير: الـ Backend غير متاح حالياً');
    const banner = document.createElement('div');
    banner.className = 'api-warning-banner';
    banner.innerHTML = `
      <div style="background-color: #fff3cd; border: 1px solid #ffc107; border-radius: 5px; padding: 12px; margin: 10px; color: #856404; text-align: center;">
        <strong>⚠️ تنبيه:</strong> الـ Backend غير متاح حالياً. قد لا تعمل بعض الميزات بشكل صحيح.
        <button onclick="this.parentElement.style.display='none'" style="background: none; border: none; cursor: pointer; float: left;">✕</button>
      </div>
    `;
